...
<script>
					...
					function fn_reply_form(url, parentNO){
						console.log(url,parentNO); //  /pro30/board/replyForm.do, 부모글 30
						var form = document.createElement("form");
...
<body>
	<c:out value="${article.articleNO}"></c:out>
		...
		<table border="0" align="center">
			<tr>
				...
				<td align="left"><input type="text"
					...
					name="parentNO" value="${article.articleNO}" /> <!-- 추후 글 수정시 글 번호 필요함 --></td>
			</tr>

			<tr>
				<td width="150" align="center" bgcolor="#FF9933">작성자 아이디</td>
				<td align="left"><input type="text" value="${article.id }"
					name="id" disabled /></td>
			</tr>

			<tr>
				<td width="150" align="center" bgcolor="#FF9933">제목</td>
				<td align="left"><input type="text" value="${article.title }"
			...
			<c:if
				...
				<tr id="imgrow">
					...
					<td><input type="hidden" name="originalFileName"
						...
						<img
						src="${contextPath}/download1.do?imageFileName=${article.imageFileName}&articleNO=${article.articleNO }"
						id="preview" /><br></td>
				</tr>
				<tr>
			...
			<tr>
				...
				<td align="left"><input type=text value="${article.writeDate}"
					disabled /></td>
			</tr>
			...
			<c:if test="${id==article.id || id == 'admin'}">
			<tr id="tr_btn" >
